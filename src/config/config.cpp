...
void TomlConfig::setDefaultValues() {
  ...
  lsm_block_cache_history_capacity_ = 2048; // Default: 2048 (history pool)
  ...
  redis_list_prefix_ = "REDIS_LIST_PREFIX_";
...
bool TomlConfig::loadFromFile(const std::string &filePath) {
  ...
  try {
    ...
    lsm_block_cache_history_capacity_ = 
        cache_config.at("LSM_BLOCK_CACHE_HISTORY_CAPACITY").as_integer();
    ...
    redis_list_prefix_ = 
      static_cast<std::string>(
        redis_config.at("REDIS_LIST_PREFIX").as_string())[0];
...
int TomlConfig::getLsmBlockCacheHistoryCapacity() const {
  return lsm_block_cache_history_capacity_;
}
...
const std::string &TomlConfig::getRedisListPrefix() const {
  return redis_list_prefix_;
}
