...
public:
  std::string data_dir; // LSMEngine 的数据存放位置，SST 文件存放在这个目录下
  MemTable memtable; // LSM Tree 引擎的内存表部分
  // level->ids, 从大到小排序，因为 sst id 越大越新
  std::map<size_t, std::deque<size_t>> level_sst_ids; // level -> ids
  std::unordered_map<size_t, std::shared_ptr<SST>> ssts; // id -> sst
  std::shared_mutex ssts_mtx; // 全局 SST 文件的访问锁，实现并发策略
  std::shared_ptr<BlockCache> block_cache; // 全局缓存池指针
  size_t next_sst_id = 0; // sst id 分配器，表示下一个 sst 位于哪
  size_t cur_max_level = 0; // 当前 SST 的最大 level

public:
  LSMEngine(std::string path, bool has_blockCache=true);
  ~LSMEngine();
