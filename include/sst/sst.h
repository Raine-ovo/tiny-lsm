...
public:
  ...
  // 找到key所在的block的idx
  std::optional<uint32_t> find_block_idx(const std::string &key);
...
private:
  ...
  size_t block_size; // 每个 block 的阈值大小
  std::shared_ptr<BloomFilter> bloom_filter;
  uint64_t min_tranc_id_ = UINT64_MAX;
  uint64_t max_tranc_id_ = 0;
...
public:
  ...
  SSTBuilder(size_t block_size, bool has_bloom, size_t target_level=0); 
  // 添加一个key-value对
