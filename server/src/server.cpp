...
private:
  ...
  std::string handleRequest(const std::string &request) {
    ...
    try {
      ...
      return "-ERR Protocal Error: invalid number of elements.";
    }
    ...
    switch (string2Ops(args[0])) {
      case OPS::PING: return "+PONG\r\n";
      case OPS::FLUSHALL: return flushall_handler(redis_);
      case OPS::SAVE: return save_handler(redis_);
      case OPS::GET: return get_handler(args, redis_);
      case OPS::SET: return set_handler(args, redis_);
      case OPS::DEL: return del_handler(args, redis_);
      case OPS::INCR: return incr_handler(args, redis_);
      case OPS::DECR: return decr_handler(args, redis_);
      case OPS::EXPIRE: return expire_handler(args, redis_);
      case OPS::TTL: return ttl_handler(args, redis_);
      case OPS::HSET: return hset_handler(args, redis_);
      case OPS::HGET: return hget_handler(args, redis_);
      case OPS::HDEL: return hdel_handler(args, redis_);
      case OPS::HKEYS: return hkeys_handler(args, redis_);
      case OPS::RPUSH: return rpush_handler(args, redis_);
      case OPS::LPUSH: return lpush_handler(args, redis_);
      case OPS::RPOP: return rpop_handler(args, redis_);
      case OPS::LPOP: return lpop_handler(args, redis_);
      case OPS::LLEN: return llen_handler(args, redis_);
      case OPS::LRANGE: return lrange_handler(args, redis_);
      case OPS::ZADD: return zadd_handler(args, redis_);
      case OPS::ZCARD: return zcard_handler(args, redis_);
      case OPS::ZINCRBY: return zincrby_handler(args, redis_);
      case OPS::ZRANGE: return zrange_handler(args, redis_);
      case OPS::ZRANK: return zrank_handler(args, redis_);
      case OPS::ZREM: return zrem_handler(args, redis_);
      case OPS::ZSCORE: return zscore_handler(args, redis_);
      case OPS::SADD: return sadd_handler(args, redis_);
      case OPS::SCARD: return scard_handler(args, redis_);
      case OPS::SMEMBERS: return smembers_handler(args, redis_);
      case OPS::SREM: return srem_handler(args, redis_);
      case OPS::SISMEMBER: return sismember_handler(args, redis_);
