...
TEST(MemTableTest, IteratorComplexOperations) {
  ...
  for (auto it = memtable.begin(0, false); it != memtable.end(); ++it) {
    result1.push_back(*it);
  }

  std::cout << result1.size() << std::endl;
  std::cout << result1[0].first << std::endl;
  std::cout << result1[0].second << std::endl;
  std::cout << result1[2].second << std::endl;
