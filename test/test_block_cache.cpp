...
protected:
  void SetUp() override {
    // 初始化缓存池，容量为3，历史池容量2，K值为2
    cache = std::make_unique<BlockCache>(3, 2, 2);
  }
...
TEST_F(BlockCacheTest, PutAndGet) {
  ...
  // 提前访问 block1 使其晋升到主缓存，避免历史池溢出导致驱逐
  cache->get(1, 1);
...
TEST_F(BlockCacheTest, CacheEviction1) {
  ...
  // 插入 block4，历史池未满，不应驱逐 block3
  cache->put(1, 4, block4);

  EXPECT_EQ(cache->get(1, 1), block1);
  EXPECT_EQ(cache->get(1, 2), block2);
  EXPECT_EQ(cache->get(1, 3), block3); // block3 未被驱逐（历史池未满）
...
TEST_F(BlockCacheTest, CacheEviction2) {
  ...
  cache->put(1, 1, block1);
  cache->put(1, 2, block2);
  // 访问 block1 和 block2 使其晋升到主缓存
  cache->get(1, 1);
  cache->get(1, 2);
  cache->put(1, 3, block3);
  cache->get(1, 3);

  // 插入并访问 block4，使其晋升到主缓存，触发主缓存容量淘汰（驱逐 LRU: block1）
  cache->put(1, 4, block4);
  cache->get(1, 4);
