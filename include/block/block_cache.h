...
class BlockCache {
public:
  BlockCache(size_t capacity, size_t history_capacity, size_t k);
  ~BlockCache();
...
private:
  size_t capacity_;          // 缓存池容量
  size_t history_capacity_;  // 访问池容量
  size_t k_;                 // LRU-K 中的 K 值
  mutable std::mutex mutex_; // 互斥锁保护缓存

  // 双向链表存储缓存项
  // 存储两个缓存队列，第一个表示访问次数大于等于 k 的，第二个表示访问次数小于 k 的
  // 大于等于 k 的是缓存池
