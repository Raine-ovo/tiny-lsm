...
// std::enabel_shared_from_this<> 允许一个对象安全地创建指向自身的 std::shared_ptr
class Block : public std::enable_shared_from_this<Block> {
  friend BlockIterator;

private:
  std::vector<uint8_t> data; // block 1. data section
  std::vector<uint16_t> offsets; // 2. offset section
  size_t capacity; // extra information section

  // data 数组中的数据，存储 kv 
  // 数据类型不一样是因为要对 entry 编码后写到 data 中
  struct Entry {
    std::string key;
    std::string value;
    uint64_t tranc_id;
...
public:
  ...
  // decode 是静态成员函数，不能直接访问类的非静态成员变量
